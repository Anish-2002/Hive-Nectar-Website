...
async function handleDone(cb) {
    ...
    if (!updateError) {
        ...
        const mobileModal = document.getElementById('missionsModal');
...
async function handleReact(btn) {
    ...
    if (error) {
        ...
        // Update local UI for mobile buttons immediately, then refresh modal if open
        const isLike = btn.dataset.type === 'like';
        const btnElement = btn;
        btnElement.style.background = isLike ? '#22c55e' : '#ef4444';
...
async function handleComment(btn) {
    ...
    // Refresh if mobile modal open, else full reload
    const mobileModal = document.getElementById('missionsModal');
    if (mobileModal && mobileModal.style.display === 'flex') {
        await refreshMobileMissionModal();
    } else {
...
async function refreshMobileMissionModal() {
    ...
    // Setup dropdowns with overflow fix
    function setupMobileFilters() {
        ...
        dropdowns.forEach(id => {
            ...
            const handler = (e) => {
                ...
                const modal = document.getElementById('missionsModal');
                if (!isOpen) {
                    if (modal) modal.style.overflowY = 'visible';
                } else {
        ...
        if (modal) {
            const closeOverflow = () => {
                if (modal.style.overflowY === 'visible') modal.style.overflowY = 'auto';
            };
            modal.addEventListener('click', closeOverflow);
            ...
            checkboxes.forEach(cb => {
                if (cb._overflowHandler) cb.removeEventListener('change', cb._overflowHandler);
                const overflowHandler = () => {
                    if (modal.style.overflowY === 'visible') modal.style.overflowY = 'auto';
                };
                cb.addEventListener('change', overflowHandler);
                cb._overflowHandler = overflowHandler;
            });
        }
    }
    setupMobileFilters();
...
function renderMobileTaskList(tasks) {
        ...
        if (commentsDiv) {
            if (taskComments.length) {
                commentsDiv.innerHTML = taskComments.map(c => `
                    <div id="mobile-comment-box-${c.id}" style="font-size:0.75rem; padding:4px 0; border-bottom:1px solid #f0f0f0; display:flex; justify-content:space-between; align-items:center;">
                        <span style="flex:1;">${c.comment_text}</span>
                        <div>
                            <button class="mobile-edit-comment" data-id="${c.id}" data-text="${c.comment_text.replace(/'/g, "\\'")}" style="background:none; border:none; color:#3b82f6; cursor:pointer; font-size:0.7rem;">Edit</button>
                            <button class="mobile-delete-comment" data-id="${c.id}" style="background:none; border:none; color:#ef4444; cursor:pointer; font-size:0.7rem; margin-left:5px;">Delete</button>
                        </div>
                    </div>
                `).join('');
            } else {
    ...
    container.querySelectorAll('.mobile-edit-comment').forEach(btn => {
        btn.addEventListener('click', (e) => {
            e.stopPropagation();
            const commentId = btn.dataset.id;
            const oldText = btn.dataset.text;
            const commentDiv = document.getElementById(`mobile-comment-box-${commentId}`);
            if (!commentDiv) return;
            commentDiv.innerHTML = `
                <input type="text" id="mobile-edit-input-${commentId}" value="${oldText}" style="flex:1; padding:4px; border:1px solid #3b82f6; border-radius:4px; font-size:0.8rem;">
                <div>
                    <button class="mobile-save-edit" data-id="${commentId}" style="background:#22c55e; color:white; border:none; padding:2px 8px; border-radius:4px; cursor:pointer;">Save</button>
                    <button class="mobile-cancel-edit" data-id="${commentId}" style="background:#64748b; color:white; border:none; padding:2px 8px; border-radius:4px; cursor:pointer; margin-left:5px;">Cancel</button>
                </div>
            `;
            commentDiv.querySelector('.mobile-save-edit').addEventListener('click', async () => {
                const newText = document.getElementById(`mobile-edit-input-${commentId}`).value;
                if (!newText.trim()) return;
                await supabase.from('task_comments').update({ comment_text: newText }).eq('id', commentId);
                if (isMobileModalOpen) await refreshMobileMissionModal();
                else initProfile();
            });
            commentDiv.querySelector('.mobile-cancel-edit').addEventListener('click', () => {
    ...
    container.querySelectorAll('.mobile-delete-comment').forEach(btn => {
